...
client.on("ready", () => {
	...
	setInterval(() => {
		client.user.setActivity(`[${PREFIX}help] | ${BUILD}`, {
			type: "PLAYING"
			...
			switch (args[0]) {
				case "answer":
					...
					try {
						...
						question_id = guestData.data.question_id;
			...
			case 'register':
				...
				if (message.channel.id == guestChannelOnly) {
					...
					if (guestChannelOnly) {
						...
						embed.setDescription(`**${message.member.displayName}**, harap verifikasi diri Anda dengan menjawab pertanyaan yang telah kami kirimkan di DM!`);
